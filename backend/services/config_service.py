...
3. **符合规则示例**：3-5条具体的话术片段或场景，覆盖不同表达方式（直接表述、委婉表述、口语化表述）
4. **不符合规则示例**：2-3条相似但不应判定命中的场景，帮助模型避免误判
...
【符合规则示例】1.示例话术或场景1；2.示例话术或场景2；3.示例话术或场景3
【不符合规则示例】1.易混淆但不命中的场景1；2.易混淆但不命中的场景2""",
        "user_prompt": """
## 原始规则描述
...
2. **严格依据规则字段判断**：
   ...
   - 参考"符合规则示例"理解命中话术的典型表达方式（包括委婉、口语化变体，不要求与示例逐字匹配）
   - 参考"不符合规则示例"排除易混淆但不应命中的情况，如果坐席表现与不符合规则示例描述的场景相似，应判定为not_matched
...
2. **严格依据规则字段判断**：
   - 参考"基础描述"确定违规的核心标准
   - 参考"触发条件"确定具体的识别逻辑，包括是否需要结合上下文多轮内容判断（若触发条件中说明需要多轮上下文，请勿仅凭孤立单句下结论）
   - 参考"符合规则示例"理解违规话术的典型表达方式（包括委婉、口语化变体，不要求与示例逐字匹配）
   - 参考"不符合规则示例"排除易混淆但不构成违规的情况，如果坐席表现与不符合规则示例描述的场景相似，应判定为not_matched
